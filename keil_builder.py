...
class KeilBuilder:
    ...
    def _parse_output_info(self) -> Tuple[Optional[str], Optional[str]]:
        ...
        try:
            ...
            match = re.search(r'"([^"]+)"\s*-\s*\d+\s*Error\(s\),\s*\d+\s*Warning\(s\)', content)
            if match:
        ...
        while not stop_event.is_set():
            try:
                if self.build_log_file.exists():
                    with open(self.build_log_file, 'r', encoding='utf-8', errors='ignore') as f:
                        f.seek(last_pos)
                        new_content = f.read()
                        if new_content:
    ...
    def _show_help(self):
        ...
        print()
        print("Options:")
        print("  -j<n>           Set parallel compilation jobs (default: -j0 = all cores)")
        ...
        print()
        print("Examples:")
        print("  python keil_builder.py                    # Auto-detect project, use -j0")
        print("  python keil_builder.py -j4                # Use 4 parallel jobs")
        print("  python keil_builder.py project.uvprojx    # Build specific project")
        print("  python keil_builder.py -j2 project.uvprojx Debug  # Build with options")
    ...
    def build(self, args: List[str]) -> int:
        ...
        print(f"[INFO] Parallel Jobs: -j{self.parallel_jobs} ({'all cores' if self.parallel_jobs == 0 else f'{self.parallel_jobs} cores'})")
        if self.project_target:
            print(f"[INFO] Target: {self.project_target}")
        print(f"[INFO] Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print("─" * 60)
        cmd = [str(self.keil_path), f'-j{self.parallel_jobs}', '-r', str(self.project_path)]
        if self.project_target:
        ...
        output_thread = threading.Thread(target=self._print_build_output, args=(stop_event,))
